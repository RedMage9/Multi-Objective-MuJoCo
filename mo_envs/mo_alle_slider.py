...
DEFAULT_INITIAL_QPOS = {
    'joint1': -0.33, #np.deg2rad(np.random.randint(low=0, high=90)),
    'joint2': 0.26, #np.deg2rad(np.random.randint(low=35, high=45)),
    'joint3': 1.3, #np.deg2rad(np.random.randint(low=35, high=45)),
    'joint4': 1.57,
    'joint5': 0.35, #np.deg2rad(np.random.randint(low=80, high=90)),
    # 'joint6': 0.002,
    'joint_0.0': 0,
    'joint_1.0': 0.,
    'joint_2.0': 0.,
...
MODEL_XML_PATH = os.path.join(os.path.abspath(os.path.dirname(__file__)), "assets/mo_alle_slider.xml")
...
class moAlleSliderEnv(robot_env.RobotEnv, utils.EzPickle):
    def __init__(
    ...
    def compute_reward(self, achieved_goal, goal, info):
        ...
        door_x = self.prev_door_x - self.sim.data.body_xpos[body_id][1]

        door_reward = door_x * -250
    ...
    def _set_action(self, action):
        ...
        body_id = self.sim.model.body_name2id('door2')
        self.prev_door_x = self.sim.data.body_xpos[body_id][1]
        ...
        mocap_ctrl = np.concatenate([mocap_pos, self.rot])

        ctrl_action1 = [action[1] for i in range(12)]
    ...
    def _get_obs(self):
        ...
        mocap_pos = self.sim.data.get_mocap_pos('mocap').copy()[1]
        
        robot_qpos, robot_qvel = robot_get_obs(self.sim)

        body_id = self.sim.model.body_name2id('door2')
        door_x = [self.sim.data.body_xpos[body_id][1]]
