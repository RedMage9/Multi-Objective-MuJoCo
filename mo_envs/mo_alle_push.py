...
DEFAULT_INITIAL_QPOS = {
    ...
    'joint2': 0.99, #np.deg2rad(np.random.randint(low=35, high=45)),
    'joint3': 2.46, #np.deg2rad(np.random.randint(low=35, high=45)),
    'joint4': 0.00,
    'joint5': -1.89, #np.deg2rad(np.random.randint(low=80, high=90)),
    # 'joint6': 0.002,
    'joint_0.0': 0,
    'joint_1.0': 0.,
    'joint_2.0': 0.,
...
MODEL_XML_PATH = os.path.join(os.path.abspath(os.path.dirname(__file__)), "assets/mo_alle_push.xml")
...
class moAllePushEnv(robot_env.RobotEnv, utils.EzPickle):
    def __init__(
    ...
    def compute_reward(self, achieved_goal, goal, info):
        ...
        box_x = self.sim.data.body_xpos[box_id][0]

        body_reward = self.prev_box_x - box_x
    ...
    def _set_action(self, action):
        ...
        box_id = self.sim.model.body_name2id('box')        
        self.prev_box_x = self.sim.data.body_xpos[box_id][0]
        ...
        # mocap_pos[0] = 0.45 - temp[0]
        # mocap_pos[1] = -temp[1]
        # mocap_pos[2] = 0.03 - temp[2]
        ...
        # ctrl_action = np.array([-robot_qpos[5], 1, 1, 1, -robot_qpos[9], 1, 1, 1, -robot_qpos[13], 1, 1, 1,0,0,0,0])
    ...
    def _get_obs(self):
        ...
        observation = np.concatenate([mocap_x, box_x])
