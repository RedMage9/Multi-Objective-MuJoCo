...
class SchmopillarEnv(robot_env.RobotEnv, utils.EzPickle):
    def __init__(self, reward_type='sparse'):
        ...
        self.scalar = np.random.rand(1) * 0.5 + 0.5
    ...
    def compute_reward(self, achieved_goal, goal, info):
        ...
        reward = obj1 * self.scalar + obj2 * (1 - self.scalar)
    ...
    def _get_obs(self):
        ...
        obs = np.concatenate([
            grip_pos[:2], bluebox1_pos.ravel()[:2], bluebox2_pos.ravel()[:2], redbox1_pos.ravel()[:2], redbox2_pos.ravel()[:2], redbox3_pos.ravel()[:2], self.scalar, 1 - self.scalar
        ])
