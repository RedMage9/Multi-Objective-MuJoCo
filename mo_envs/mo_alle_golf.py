...
DEFAULT_INITIAL_QPOS = {
    ...
    'joint2': 0.6, #np.deg2rad(np.random.randint(low=35, high=45)),
    'joint3': 1.32, #np.deg2rad(np.random.randint(low=35, high=45)),
    'joint4': -0.0,
    'joint5': -0.36, #np.deg2rad(np.random.randint(low=80, high=90)),
    'joint6': -0.,
    'joint_0.0': 0,
    'joint_1.0': 0.3,
    'joint_2.0': 0.3,
...
MODEL_XML_PATH = os.path.join(os.path.abspath(os.path.dirname(__file__)), "assets/mo_alle_golf.xml")
...
class moAlleGolfEnv(robot_env.RobotEnv, utils.EzPickle):
    def __init__(
    ...
    def compute_reward(self, achieved_goal, goal, info):
        ...
        ext = -np.linalg.norm(ball_pos[:2] - target_pos[:2], axis=-1) 
        # obj1 = (self.max_ball_hieght - 0.06) * 5 + ext
        # obj2 = (self.max_ball_hieght - 0.06) * -5 + ext

        obj1 = (ball_pos[2] - 0.04) * 50 + ext
        obj2 = (ball_pos[2] - 0.04) * -50 + ext
        ...
        # print(self.sim.data.get_site_xpos('clubtip'))
        # print(reward)
    ...
    def _set_action(self, action):
        ...
        # mocap_ctrl = action[1:]
        # mocap_ctrl = np.concatenate([[7. - mocap_pos[0] * 10], action[1:]])  
        mocap_ctrl = np.concatenate([[35. - mocap_pos[0] * 50], [0. - mocap_pos[1] * 50], [action[1]]])  
        # mocap_ctrl = np.concatenate([action[0:2], [1.75 - mocap_pos[2] * 10]])  
        # mocap_ctrl = np.concatenate([[7. - mocap_pos[0] * 10], [0.], [1.75 - mocap_pos[2] * 10]])
        # mocap_ctrl[0] = 40 - mocap_pos[0] * 100
        # mocap_ctrl[2] = 3.75 - mocap_pos[2] * 10
        mocap_ctrl = mocap_ctrl * 0.02

        # rot = self.sim.data.get_mocap_quat('mocap') 
        mocap_ctrl = np.concatenate([mocap_ctrl, self.rot])
        joint6_action = action[0]
    ...
    def _viewer_setup(self):
        ...
        body_id = self.sim.model.body_name2id('ball')
