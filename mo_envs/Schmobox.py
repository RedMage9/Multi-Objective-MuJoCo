...
MODEL_XML_PATH = os.path.join(os.path.abspath(os.path.dirname(__file__)), "assets/Schmobox.xml")

def goal_distance(goal_a, goal_b):
    assert goal_a.shape == goal_b.shape
    return np.linalg.norm(goal_a - goal_b, axis=-1)

class SchmoboxEnv(robot_env.RobotEnv, utils.EzPickle):
    def __init__(self, reward_type='sparse'):
        ...
        super(SchmoboxEnv, self).__init__(
    ...
    def compute_reward(self, achieved_goal, goal, info):
        ...
        obj1 = 0.5 - np.linalg.norm(bluebox1_xy - np.array([0.7, 0]), axis=-1) - np.linalg.norm(bluebox2_xy - np.array([0.7, 0]), axis=-1)

        obj2 = - np.linalg.norm(redbox1_xy - self.initial_redbox1_xy, axis=-1) - np.linalg.norm(redbox2_xy - self.initial_redbox2_xy, axis=-1) - np.linalg.norm(redbox3_xy - self.initial_redbox3_xy, axis=-1) 
    ...
    def _set_action(self, action):
        ...
        # previous red boxes positions
        
        # self.prev_redbox1_xy = self.sim.data.get_site_xpos('redbox1')[:2]
        # self.prev_redbox2_xy = self.sim.data.get_site_xpos('redbox2')[:2]
        # self.prev_redbox3_xy = self.sim.data.get_site_xpos('redbox3')[:2]
    ...
    def _get_obs(self):
        ...
        obs = np.concatenate([
            grip_pos[:2], gripper_state, bluebox1_pos.ravel() , bluebox2_pos.ravel() , redbox1_pos.ravel() , redbox2_pos.ravel() , redbox3_pos.ravel()
        ])
    ...
    def _sample_goal(self):
        ...
        self.initial_redbox1_xy = self.sim.data.get_site_xpos('redbox1')[:2].copy()
        self.initial_redbox2_xy = self.sim.data.get_site_xpos('redbox2')[:2].copy()
        self.initial_redbox3_xy = self.sim.data.get_site_xpos('redbox3')[:2].copy()
