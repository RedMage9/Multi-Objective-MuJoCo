...
MODEL_XML_PATH = os.path.join(os.path.abspath(os.path.dirname(__file__)), "assets/mo_alle_drawer.xml")
...
class moAlleDrawerEnv(robot_env.RobotEnv, utils.EzPickle):
    def __init__(
    ...
    def compute_reward(self, achieved_goal, goal, info):
        ...
        door_joint = (0.76 - self.sim.data.get_joint_qpos('door_joint').copy()[0]) * 20
        # door_joint = door_joint - self.prev_door_joint
        # print(door_joint)

        # mocap_quat = self.sim.data.get_mocap_quat('mocap') 
        ...
        obj1 = door_joint + (finger_grip * -2) - np.linalg.norm(self.sim.data.get_site_xpos('doornoob') - mocap_pos, axis=-1)
        obj2 = door_joint + (np.linalg.norm(robot_qpos[5:].copy(), axis=-1) * -0.5) - np.linalg.norm(self.sim.data.get_site_xpos('doornoob') - mocap_pos, axis=-1)
    ...
    def _get_obs(self):
        ...
        robot_qpos, robot_qvel = robot_get_obs(self.sim)
        door_joint = [self.sim.data.get_joint_qpos('door_joint').copy()[0]]
