...
DEFAULT_INITIAL_QPOS = {
    ...
    'joint3': 2.27, #np.deg2rad(np.random.randint(low=35, high=45)),
    'joint4': 0.00,
    'joint5': -1.11, #np.deg2rad(np.random.randint(low=80, high=90)),
    # 'joint6': 0.002,
    'joint_0.0': 0,
    'joint_1.0': 0.,
    'joint_2.0': 0.,
...
MODEL_XML_PATH = os.path.join(os.path.abspath(os.path.dirname(__file__)), "assets/mo_alle_basket.xml")
...
class moAlleBasketEnv(robot_env.RobotEnv, utils.EzPickle):
    def __init__(
        ...
        self.rot = [1., 0., 1., 0.]
        self.n_actions = 5
        self.hand_start_pos = np.array([0.45, 0., 0.2])
    ...
    def compute_reward(self, achieved_goal, goal, info):
        ...
        body_id = self.sim.model.body_name2id('basket')
        basket_x = self.sim.data.body_xpos[body_id][0]
        
        body_reward = self.prev_basket_x - basket_x

        obj1 = body_reward * 300
    ...
    def _set_action(self, action):
        ...
        body_id = self.sim.model.body_name2id('basket')
        self.prev_basket_x = self.sim.data.body_xpos[body_id][0]
        ...
        ctrl_action1 = [action[2] for i in range(4)]
        ctrl_action2 = [action[3] for i in range(4)]
        ctrl_action3 = [action[4] for i in range(4)]
        ctrl_action4 = [1 for i in range(4)]
        ctrl_action = np.concatenate([ctrl_action1, ctrl_action2, ctrl_action3, ctrl_action4])
    ...
    def _get_obs(self):
        ...
        observation = np.concatenate([mocap_pos, robot_qpos[5:17].copy(), basket_x])
